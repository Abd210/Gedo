        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {images.map((img, i) => (
            <button key={img.id} className="relative group" onClick={() => { setIdx(i); setOpen(true); }}>
              <img src={img.url} alt={img.caption || 'photo'} className="w-full h-32 md:h-40 object-cover rounded shadow-card group-hover:shadow-hover transition" />
            </button>
          ))}
        </div>
      </div>
      ...
      {open && (
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center" onClick={() => setOpen(false)}>
          <div className="relative max-w-3xl w-full px-4" onClick={(e) => e.stopPropagation()}>
            <img src={images[idx]?.url} alt={images[idx]?.caption || 'photo'} className="w-full max-h-[80vh] object-contain rounded" />
